    ...
    public class PurchaseDocumentService : IPurchaseDocumentService
    {
        ...
        private void ComposeOrderDeliveryInfo(IContainer container, Domain.Entities.PurchaseOrder order)
        {
            container.Row(row =>
            {
                row.RelativeItem().Column(column =>
                {
                    column.Item().Text("INFORMACI�N DE ENTREGA").Bold().FontSize(11);

                    column.Item().PaddingTop(5).Row(r =>
                    {
                        r.AutoItem().Text("Almac�n: ").Bold();
                        r.AutoItem().Text(order.Warehouse.Name);
                    });

                    column.Item().Row(r =>
                    {
                        r.AutoItem().Text("C�digo: ").Bold();
        ...
        private void ComposeOrderItemsTable(IContainer container, Domain.Entities.PurchaseOrder order)
        {
            container.Table(table =>
            {
                ...
                table.ColumnsDefinition(columns =>
                {
                    ...
                    columns.ConstantColumn(100); // C�digo
                ...
                table.Header(header =>
                {
                    ...
                    header.Cell().Element(CellStyle).Text("C�digo").Bold();
                    header.Cell().Element(CellStyle).Text("Descripci�n").Bold();
        ...
        private void ComposeOrderTerms(IContainer container)
        {
            ...
            {
                column.Item().Text("T�rminos y Condiciones").Bold().FontSize(9);
                column.Item().PaddingTop(5).Text(
                    "1. Los precios est�n sujetos a cambio sin previo aviso.\n" +
                    "2. El tiempo de entrega puede variar seg�n disponibilidad.\n" +
                    "3. La mercanc�a debe ser inspeccionada al momento de la entrega.\n" +
                    "4. No se aceptan devoluciones despu�s de 30 d�as."
        ...
        #region RECIBO DE MERCANC�A PDF
            ...
            container.Row(row =>
            {
                ...
                row.ConstantItem(200).Column(column =>
                {
                    column.Item().Background("#FEF3E5") // Color secundario claro
                        ...
                        .Column(innerColumn =>
                        {
                            innerColumn.Item().Text("RECIBO DE MERCANC�A")
                                .FontSize(14)
                                .Bold()
                                .FontColor("#1D336F"); // Color primario
                            ...
                            // Mostrar si ya se aplic� al inventario
        ...
        private void ComposeReceivingInfo(IContainer container, Domain.Entities.PurchaseOrderReceiving receiving)
        {
            container.Column(column =>
            {
                ...
                column.Item().PaddingTop(10).Row(row =>
                {
                    ...
                    row.RelativeItem().Column(col =>
                    {
                        col.Item().Text("ALMAC�N").Bold().FontSize(11).FontColor("#1D336F");
        ...
        private void ComposeReceivingItemsTable(IContainer container, Domain.Entities.PurchaseOrderReceiving receiving)
        {
            container.Table(table =>
            {
                table.ColumnsDefinition(columns =>
                {
                    ...
                    columns.ConstantColumn(90);  // C�digo
        ...
        private void ComposeReceivingSummary(IContainer container, Domain.Entities.PurchaseOrderReceiving receiving)
        {
            ...
            container.Background(Colors.Grey.Lighten4)
                ...
                .Row(row =>
                {
                    row.RelativeItem().Column(column =>
                    {
                        column.Item().Text("RESUMEN DE RECEPCI�N").Bold().FontSize(11);

                        column.Item().PaddingTop(5).Row(r =>
                        {
                            r.AutoItem().Text("Total Art�culos: ").Bold();
        ...
        private void ComposeReceivingSignatures(IContainer container)
        {
            container.Row(row =>
            {
                row.RelativeItem().Column(column =>
                {
                    ...
                    column.Item().PaddingTop(5).AlignCenter().Text("Recibi�").FontSize(9).Bold();
                    column.Item().AlignCenter().Text("Almac�n").FontSize(8);
                });
                ...
                {
                    column.Item().LineHorizontal(1).LineColor(Colors.Grey.Medium);
                    column.Item().PaddingTop(5).AlignCenter().Text("Autoriz�").FontSize(9).Bold();
        ...
        private void ComposeFooter(IContainer container)
        {
            ...
            {
                text.Span("Documento generado autom�ticamente el ").FontSize(8).FontColor(Colors.Grey.Darken1);
