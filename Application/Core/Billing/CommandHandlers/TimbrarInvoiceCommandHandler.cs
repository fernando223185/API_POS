    ...
    public class TimbrarInvoiceCommandHandler : IRequestHandler<TimbrarInvoiceCommand, InvoiceResponseDto>
    {
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                // 6. Si MetodoPago es PPD, inicializar campos de cuentas por cobrar
                if (invoice.MetodoPago == "PPD")
                {
                    Console.WriteLine($"   📋 Factura PPD - inicializando campos de cuentas por cobrar...");
                    
                    invoice.DueDate = (invoice.TimbradoAt ?? invoice.InvoiceDate).AddDays(30); // 30 días por defecto
                    ...
                    Console.WriteLine($"   ✓ Campos PPD inicializados - Balance: ${invoice.Total:N2}, Vencimiento: {invoice.DueDate:yyyy-MM-dd}");
                }

                // 7. Guardar Invoice completa
