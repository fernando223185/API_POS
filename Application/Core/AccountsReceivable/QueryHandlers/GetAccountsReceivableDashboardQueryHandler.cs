...
public class GetAccountsReceivableDashboardQueryHandler : IRequestHandler<GetAccountsReceivableDashboardQuery, AccountsReceivableDashboardDto>
{
    ...
    public GetAccountsReceivableDashboardQueryHandler(IInvoicePPDRepository invoiceRepository)
    {
    ...
    public async Task<AccountsReceivableDashboardDto> Handle(GetAccountsReceivableDashboardQuery request, CancellationToken cancellationToken)
    {
        ...
        var totalReceivable = invoices.Sum(i => i.BalanceAmount);
        var totalOverdue = invoices.Where(i => i.DueDate < today).Sum(i => i.BalanceAmount);
        var totalDueToday = invoices.Where(i => i.DueDate == today).Sum(i => i.BalanceAmount);
        var totalNotDue = invoices.Where(i => i.DueDate > today).Sum(i => i.BalanceAmount);

        // Reporte de antigüedad
        var aging = new AgingReportDto
        {
            Current = invoices.Where(i => i.DaysOverdue <= 0).Sum(i => i.BalanceAmount),
            Days31To60 = invoices.Where(i => i.DaysOverdue >= 31 && i.DaysOverdue <= 60).Sum(i => i.BalanceAmount),
            Days61To90 = invoices.Where(i => i.DaysOverdue >= 61 && i.DaysOverdue <= 90).Sum(i => i.BalanceAmount),
            Over90Days = invoices.Where(i => i.DaysOverdue > 90).Sum(i => i.BalanceAmount),
            CurrentCount = invoices.Count(i => i.DaysOverdue <= 0),
            Days31To60Count = invoices.Count(i => i.DaysOverdue >= 31 && i.DaysOverdue <= 60),
            Days61To90Count = invoices.Count(i => i.DaysOverdue >= 61 && i.DaysOverdue <= 90),
            Over90DaysCount = invoices.Count(i => i.DaysOverdue > 90)
        };
        ...
        var topCustomers = invoices
            ...
            .Select(g => new TopCustomerBalanceDto
            {
                ...
                OverdueAmount = g.Where(i => i.DaysOverdue > 0).Sum(i => i.BalanceAmount),
